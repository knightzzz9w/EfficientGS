...
class BackEnd(mp.Process):
    def __init__(self, config):
        ...
        self.submap_mapping_iter = config["Training"]["submap_mapping_iter"]
    ...
    def submap_mapping(self, exceeds_kf_id, iters=100):
        if len(exceeds_kf_id) == 0:
            return
        map_time = TicToc() ; map_time.tic()
        viewpoint_stack = [self.viewpoints[kf_idx.item()] for kf_idx in exceeds_kf_id]
        len_id = len(exceeds_kf_id)

        #tqdm for submap mapping
        print("begin submap mapping rendering ")
        for _ in tqdm(range(iters)):
            ...
            keyframes_opt = []
            #random choose 8 len_id
             
            if len_id > 8:
                cur_ids = torch.randperm(len_id)[:8]
            else:
                cur_ids = range(len_id)

            for cam_idx in cur_ids:
                viewpoint = viewpoint_stack[cam_idx]
                render_pkg = render(
                    viewpoint, self.gaussians, self.pipeline_params, self.background, kernel_size=self.kernel_size
                )
        ...
        with torch.no_grad():
            for kf_idx in self.occ_aware_visibility.keys():
            #render new frame and only update the occ
                ...
                self.occ_aware_visibility[kf_idx] = (n_touched > 0).long()
        print("end submap mapping rendering ")
        return gaussian_split
    ...
    def push_to_frontend_gaussian(self, tag=None):
        self.last_sent = 0
        if tag is None:
            tag = "sync_backend"
        ...
        while True:
            if self.backend_queue.empty():
                ...
                if data[0] == "stop":
                    ...
                    exceed_kf_ids = data[1]
                    self.submap_mapping(exceed_kf_ids , iters=self.submap_mapping_iter)
                    #print("keyframe mapping time is " , kf_time.toc())
                    self.push_to_frontend_gaussian("submap")
                    ...
                    self.viewpoints[cur_frame_idx] = viewpoint
                    print("add key frame id is " , cur_frame_idx)
